...
> **⚠️ 重要な注意事項 (2026-01-25更新)**
>
> このドキュメントは**古いアーキテクチャ設計書**であり、**現在の実装とは一致していません**。
> 一部の設計概念（OPUS-MT）は廃止され、NLLB-200に置き換えられています。
>
> **現在の実装状況 (v0.2.x)**:
> - ✅ **NLLB-200エンジン**: ローカル翻訳実装済み（CTranslate2最適化、gRPC通信）
> - ✅ **Google Gemini**: Cloud AI翻訳実装済み（Relay Server経由）
> - ✅ **フォールバック機能**: Gemini → NLLB-200フォールバック実装済み
> - ✅ **複数エンジン切り替え**: 設定画面で切り替え可能
>
> **現在の翻訳システム詳細は `CLAUDE.md` の「gRPC Translation System」セクションを参照してください。**
