...
Baketa プロジェクトのアダプターレイヤーには、オーバーライドを意図していないクラスが存在します。例えば `DefaultWindowsImageAdapter` は基本実装として提供されており、派生クラスでのオーバーライドを想定していません。

このようなクラスを拡張したい場合は：

1. 直接継承するのではなく、同じインターフェース（例: `IWindowsImageAdapter`）を実装する新しいクラスを作成
2. 必要に応じて処理を委譲する方法を検討
3. Composition Over Inheritance（継承よりもコンポジション）の原則に従う

### 拡張性の理由

一部の実装クラスがオーバーライドを想定していない理由：
