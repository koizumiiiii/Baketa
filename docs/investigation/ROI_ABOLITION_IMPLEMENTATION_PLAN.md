...
### **Phase 1: 緊急対応 - 現行システム動作復旧** ⭐⭐⭐⭐⭐
**優先度**: P0（最優先）
**理由**: 現在ROI画像破損により翻訳処理に到達できない状態

- [ ] **1.1 Phase 10.40画像破損問題の解決**
  - [ ] Geminiレビュー結果の確認（Background Bash 647295, f9e728）
  - [ ] 推奨される修正方針の実装
  - [ ] 動作確認（翻訳処理が正常に実行されることを確認）
  - **工数**: 0.5-1日
  - **担当**: Phase 10.40調査結果に基づく
...
### **Phase 4: ROIBasedCaptureStrategy廃止** ⭐⭐⭐
**優先度**: P1
...
- [ ] **4.2 ROIBasedCaptureStrategy削除**
  - ファイル削除: `ROIBasedCaptureStrategy.cs`
  - 関連する設定削除
  - 未使用コード削除
  - **工数**: 0.5日

- [ ] **4.3 不要になったコード・ファイルの削除**
  - **削除対象ファイル**:
...
### **Phase 5: 追加最適化（オプション）** ⭐⭐
**優先度**: P2
**前提条件**: Phase 4完了、かつ目標10-15秒未達成の場合のみ
...
| Phase | 内容 | 工数 | 累積工数 |
|-------|------|------|---------|
| **Phase 1** | 緊急対応（現行システム復旧） | 1-1.5日 | 1-1.5日 |
| **Phase 2** | 全画面OCR方式実装 | 2-2.5日 | 3-4日 |
| **Phase 3** | 動作検証・パフォーマンス測定 | 2.5日 | 5.5-6.5日 |
| **Phase 4** | ROI方式廃止 | 1日 | 6.5-7.5日 |
| **Phase 5** | 追加最適化（オプション） | 0.5-1.5日 | 7-9日 |

**総実装工数**: **6.5-9日**（Phase 5除く: 6.5-7.5日）
