...
## 🔧 **P0: 画像変化検知システム** 🔄 **実装中 - 深刻なキャプチャ問題発覚**

### **現状分析 (2025-01-21更新)**
- ✅ **P0: Image Change Detection System**基盤実装済み (`EnhancedImageChangeDetectionService.cs`)
  - ✅ 4種類のPerceptual Hash (AverageHash, DifferenceHash, PerceptualHash, WaveletHash)
  - ✅ 3段階フィルタリング (Stage1: 90%+フィルタリング, Stage2: 8%処理, Stage3: 2%処理)
  - ✅ `AdaptiveCaptureService.CaptureAsync()`統合完了
- ❌ **重大問題発覚**: Windows Graphics Capture部分失敗でGDIフォールバックが**白い画像**返却
- ❌ **ROI高解像度キャプチャエラー**: `System.Drawing.Graphics.DrawImage`の例外発生
...
### **🎯 最優先: P0 画像変化検知システム**
**理由**: P1段階的フィルタリングシステムは完成したが、画像変化検知(Stage 1)は既存のImageChangeDetectionStageStrategyで暫定実装のため、本格実装が必要。

**期待効果**: 
- OCR実行回数: 85%削減
- Stage 1での早期終了による更なる高速化
- 現在の90.5%削減をさらに上回る最適化
...
### **🎯 Phase 0: WGC修復優先アプローチ (修正版)**

**根拠**: ログ分析により、WGCは部分的に動作しているが**常に白い画像 (`FFFFFFFF`) を返している**ことが判明。Graphics Hooking実装前にWGC修復を試みることでコストパフォーマンスを最大化。

#### **優先順位修正版**
1. **WGC部分的失敗の詳細調査** (2日) 🔍
   - ROI高解像度処理エラーの根本原因特定
   - `System.Drawing.Graphics.DrawImage`例外の詳細解析  
   - メモリ管理・リソース競合問題の特定

2. **WGC修復実装** (2日) 🛠️
   - 特定された問題の修正実装
   - ROI処理パイプラインの最適化
   - **期待成果**: 60-80%のケースでWGC完全復旧

3. **フォールバック戦略完全実装** (2日) 🏗️  
   - `GDIFallbackStrategy` スタブ → 完全実装
   - `PrintWindowFallbackStrategy` 直接API実装
   - **期待成果**: WGC失敗時の確実な代替手段

4. **Graphics Hooking実装** (Phase 1に移動) 🚀
   - **位置付け変更**: 緊急対応 → 追加最適化手段
   - **実装タイミング**: WGC修復成果確認後

### **期待効果見込み**
- **短期 (1週間後)**: キャプチャ成功率 0% → 70-85%
- **中期 (Phase 1完了後)**: 90-95% (Graphics Hooking追加)
- **コスト削減**: Graphics Hooking先行実装に比べ50%のリスク削減

**実装開始推奨時期**: 即座 (WGC修復優先)
**修復期間**: 6日 (WGC修復 4日 + フォールバック実装 2日)
**総開発期間**: 4週間 (WGC修復 1週間 + Graphics Hooking 2週間 + 統合最適化 1週間)
**投資対効果**: 極めて高い (段階的リスク軽減により確実性向上)
