...
- **SentencePiece統合**: Microsoft.ML.Tokenizers v0.21.0による高精度トークン化
- **差分検出**: 画面変更の高精度検出によるパフォーマンス最適化
- **多言語翻訳**: ローカル（OPUS-MT）とクラウド（Gemini API）のフォールバック対応
- **中国語翻訳システム**: 簡体字・繁体字・双方向翻訳完全対応
...
- **SentencePiece統合**: OPUS-MTモデルとの完全互換性と高精度トークン化
- **フォールバック翻訳**: ローカル（高速・無料）とクラウド（高品質・有料）のフォールバック対応
- **東アジア3言語完全対応**: 日本語↔英語↔中国語の完全相互翻訳
...
- **SentencePiece統合**: 高精度トークン化とモデル互換性
- **中国語翻訳**:
  - 簡体字（中国本土）↔ 繁体字（台湾・香港）
  - 自動文字体系検出機能
  - プレフィックス自動付与（OPUS-MT特殊トークン）
- **双方向翻訳戦略**:
  - 直接翻訳: zh↔en, zh→ja, ja↔en
  - 2段階翻訳: ja→en→zh（高品質）
- **フォールバック戦略**: レート制限・ネットワークエラー時のフォールバック対応
