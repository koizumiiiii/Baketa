-- ============================================================
-- ⚠️ [OBSOLETE] このスクリプトは廃止されました
-- ============================================================
-- Issue #347: ボーナストークンの有効期限が削除されたため、
-- このマイグレーションスクリプトは使用できません。
--
-- 新規移行が必要な場合は、以下の点に注意してください:
-- - bonus_tokens テーブルに expires_at カラムは存在しません
-- - ボーナストークンは永続的に有効です
-- - grant_bonus_tokens() 関数の呼び出しに p_expires_at は不要です
--
-- 参照: docs/database/migrations/347_remove_bonus_token_expiration.sql
-- ============================================================

-- ============================================================
-- [LEGACY] Issue #280+#281 Phase 4: 既存プロモーション → ボーナストークン移行
-- ============================================================
--
-- 実行前の確認事項:
...
-- - [OBSOLETE] 有効期限は redeemed_at + duration_days で計算 → Issue #347で削除
