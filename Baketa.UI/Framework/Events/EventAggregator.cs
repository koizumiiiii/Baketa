...
namespace Baketa.UI.Framework.Events;
    ...
    internal sealed class EventAggregator(ILogger<EventAggregator>? logger = null) : IEventAggregator
    {
        ...
        /// <inheritdoc/>
        public async Task PublishAsync<TEvent>(TEvent eventData) where TEvent : Baketa.Core.Abstractions.Events.IEvent
        {
        ...
        /// <inheritdoc/>
        public IDisposable Subscribe<TEvent>(Func<TEvent, Task> handler) where TEvent : Baketa.Core.Abstractions.Events.IEvent
        {
            var eventType = typeof(TEvent);
            if (_logger != null)
