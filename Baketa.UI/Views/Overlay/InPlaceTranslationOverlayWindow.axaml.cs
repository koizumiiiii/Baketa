...
#pragma warning restore SYSLIB1054
    ...
    private static bool IsCorruptedOrErrorTranslation(string? translatedText)
    {
        ...
        // 🚨 Helsinki-NLP/opus-mt-en-japモデル汚染パターン検出
        var corruptedPatterns = new[]
        {
            "オベル",     // "Hello" -> "オベル," の主要汚染パターン
            "テマ",       // "テマ の 子 ら は 奪 わ れ ず" パターン
            "ハマテ",     // "ハマ テ で あ っ た" パターン
            "ピハヒロテ", // "ピハヒロテは" パターン
            "マグブキ",   // 既知の汚染パターン
        };
        ...
        // ランダムな数字や記号のみの文字列
        if (System.Text.RegularExpressions.Regex.IsMatch(translatedText, @"^[0-9\s\-\.~\p{P}]+$"))
        {
            return true; // "2473~928" のようなパターン
        }
