...
/// <summary>
/// Core.Abstractions版のIWindowManagerAdapterインターフェースの完全実装
/// Windows Graphics Capture API、PrintWindow、サムネイル生成などを統合
/// </summary>
public class CoreWindowManagerAdapter : Baketa.Core.Abstractions.Platform.Windows.Adapters.IWindowManagerAdapter
{
    private readonly Baketa.Core.Abstractions.Platform.Windows.IWindowManager _windowManager;

    public CoreWindowManagerAdapter(Baketa.Core.Abstractions.Platform.Windows.IWindowManager windowManager)
    {
        _windowManager = windowManager ?? throw new ArgumentNullException(nameof(windowManager));
        Console.WriteLine("🚀 CoreWindowManagerAdapter コンストラクタ呼び出し完了");
    }
    ...
    {
        return false; // 現在の実装では型変換をサポートしない
    }
    ...
    public IReadOnlyCollection<Baketa.Core.Abstractions.Platform.Windows.Adapters.WindowInfo> GetRunningApplicationWindows()
    {
        try
        {
            // 🚀 UltraThink修正: デバッグログ追加で原因特定
            Console.WriteLine("🔍 CoreWindowManagerAdapter: GetRunningApplicationWindows開始");

            // 基盤のIWindowManagerを使ってウィンドウを取得
            var windows = _windowManager.GetRunningApplicationWindows();
            Console.WriteLine($"🔍 CoreWindowManagerAdapter: WindowsManagerから{windows.Count}個のウィンドウを取得");
            ...
            foreach (var window in windows)
            {
                ...
                Console.WriteLine($"🔍 CoreWindowManagerAdapter: 候補ウィンドウ検査 - '{title}' (Handle: {handle})");

                // 空のタイトルをスキップ
                if (string.IsNullOrWhiteSpace(title))
                {
                    Console.WriteLine($"⚠️ CoreWindowManagerAdapter: 空タイトルでスキップ - Handle: {handle}");
                    continue;
                }
                ...
                {
                    Console.WriteLine($"⚠️ CoreWindowManagerAdapter: Baketaアプリで除外 - '{title}'");
                    continue;
                }
                ...
                if (isMinimized)
                {
                    Console.WriteLine($"⚠️ CoreWindowManagerAdapter: 最小化ウィンドウで除外 - '{title}' (Handle: {handle})");
                    // 最小化ウィンドウは一旦除外せずに含める（UIで判断）
                    // continue;
                }
                ...
                {
                    Console.WriteLine($"⚠️ CoreWindowManagerAdapter: 画面外ウィンドウで除外 - '{title}' Bounds: {bounds}");
                    continue;
                }
                ...
                {
                    Console.WriteLine($"⚠️ CoreWindowManagerAdapter: 無効状態で除外 - '{title}' Visible: {isVisible}, Size: {bounds.Width}x{bounds.Height}");
                    continue;
                }

                // 🚀 UltraThink緊急修正: サムネイル生成前の事前フィルタリング
                if (!ShouldIncludeWindow(title, bounds))
                {
                    Console.WriteLine($"⚠️ CoreWindowManagerAdapter: 事前フィルタリングで除外 - '{title}' Size: {bounds.Width}x{bounds.Height}");
                    continue;
                }
                ...
                Console.WriteLine($"✅ CoreWindowManagerAdapter: ウィンドウ追加成功 - '{title}' (Handle: {handle})");
            }

            Console.WriteLine($"✅ CoreWindowManagerAdapter: 最終結果 - {windowList.Count}個のウィンドウをリストに追加");
            ...
            Console.WriteLine($"✅ CoreWindowManagerAdapter: GetRunningApplicationWindows完了 - {result.Count}個のウィンドウを返却");
            return result;
        }
        catch (Exception ex)
        {
...
#pragma warning restore CS8625
    ...
    private static bool ShouldIncludeWindow(string title, Rectangle bounds)
    {
        // 空タイトルは除外
        if (string.IsNullOrWhiteSpace(title))
        {
        ...
        // システムウィンドウ系の除外
        if (IsSystemOrBackgroundWindow(title))
        {
            return false;
        }

        // 開発ツール・デバッガーの除外
        if (IsDeveloperToolWindow(title))
        {
            return false;
        }
    ...
    private static bool IsSystemOrBackgroundWindow(string title)
    {
        var systemPatterns = new[]
        {
            ...
            "Windows Security", "Windows Defender", "Antimalware Service",
    ...
    private static bool IsDeveloperToolWindow(string title)
    {
        ...
        {
            "Visual Studio", "JetBrains", "IntelliJ", "Eclipse",
            "DevTools", "Developer Tools", "Debug", "Debugger",
            "Console", "Terminal", "PowerShell", "Command Prompt",
